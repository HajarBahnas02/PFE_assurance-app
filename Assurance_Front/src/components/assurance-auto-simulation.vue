        ...
        <form @submit.prevent="submitForm">
          ...
          <div class="radio-wrapper">
            <div
              ...
              :class="[
                'radio-option',
                { selected: form.type_motorisation_id === typeMotorisation.id },
              ]"
            >
              <input
                type="radio"
                ...
                required
              />
              <label :for="'motorisation_' + typeMotorisation.id">{{
                typeMotorisation.typeM
              }}</label>
            </div>
          </div>
          <span v-if="errors.type_motorisation_id" class="error">{{
            errors.type_motorisation_id
          }}</span>
          ...
          <div>
            ...
            <select v-model="form.puissanceFiscale" id="puissanceFiscale">
              <option
                v-for="puissance in puissancesfisc"
                :key="puissance.id"
                :value="puissance.id"
              >
            ...
            <span v-if="errors.puissanceFiscale" class="error">{{
          ...
          <div>
            ...
            <span v-if="errors.dateMiseEnCirculation" class="error">{{
          ...
          <div>
            ...
            <span v-if="errors.valeurVenale" class="error">{{
              errors.valeurVenale
            }}</span>
          </div>
      ...
      <div class="form-client">
        ...
        <form @submit.prevent="submitForm">
          ...
          <div>
            ...
            <span v-if="errorsCl.date_naissance" class="error">{{
              errorsCl.date_naissance
            }}</span>
          </div>
          <div>
...
.form-vehicule input:not(:placeholder-shown),
.form-client input:not(:placeholder-shown),
.form-vehicule select:valid,
.form-client select:valid {
  border-color: green; /* Bordure verte quand rempli */
}

.form-vehicule input:hover,
.form-client input:hover,
.form-vehicule select:hover,
.form-client select:hover {
  border-color: green;
}

.form-vehicule input:focus,
.form-client input:focus,
.form-vehicule select:focus,
...
>>>>>>> c9b0abd00397548292d5401aa06832261787650c
  },
  created() {
    this.fetchData();
    this.fetchVilles();
  ...
  methods: {
    ...
    fetchData() {
      ...
      axios
        ...
        .catch((error) => {
          console.error(
            "Erreur lors de la récupération des types de motorisation:",
            error
          );
        });
